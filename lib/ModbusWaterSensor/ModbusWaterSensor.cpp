...
void ModbusSensor::requestData()
{
  ...
  unsigned int crc = getCRC16(_readCommand, 6);
  _readCommand[6] = crc & 0xFF;       // least significant CRC
  _readCommand[7] = crc >> 8;         // most significant CRC
...
byte ModbusSensor::receiveData()
{
  ...
  unsigned int crc = getCRC16(responseData, 7);

  // verify integrity of the data
  if (responseData[7] != crc & 0xFF || responseData[8] != crc >> 8)
  {
...
unsigned int ModbusSensor::getCRC16(const byte *data, unsigned int size)
{
...
float EC_TDS::readValue()
{
  ...
  // wait 200 ms for sensor response
  delay(200);

  // receive data from sensor
  byte error = receiveData();
  float value = (error == NO_ERROR) ? (_rawData[2] * 256 + _rawData[3]) / 10 : 0.0;
...
float EC_TDS::readCalibrationValue()
{
  ...
  // receive data from sensor
  byte error = receiveData();
  float value = (error == NO_ERROR) ? (_rawData[0] * 256 + _rawData[1]) / 10 : 0.0;
